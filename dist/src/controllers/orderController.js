    ...
    async getLastSixMonthsTotals(req, res) {
        try {
            ...
            const data = series.map(m => {
                ...
                return { ...m, grossTotal: gross, netTotal: net, discountTotal: discount, total: net, productCost: Math.round(costMap[m.ym] || 0) };
            });
    ...
    async getOrdersByCounter(req, res) {
        try {
            ...
            const query = `
                ...
                LEFT JOIN counters c ON o.counterID = c.id
                LEFT JOIN users u ON o.user_id = u.id
                LEFT JOIN users rep ON c.RepID = rep.id
                WHERE o.counterID = ?
                ORDER BY o.orderDate DESC, o.id DESC
            `;
