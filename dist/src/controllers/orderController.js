...
class OrderController {
    ...
    async getOrdersByMonths(req, res) {
        try {
            const { months, payment, counterId, search, repId } = req.query;
            if (!months) {
            ...
            // status column not present; skip
            ...
            return res.status(200).json({ success: true, message: 'Monthly orders (summary) fetched', data, count: data.length, meta: { months: monthList } });
        } catch (error) {
