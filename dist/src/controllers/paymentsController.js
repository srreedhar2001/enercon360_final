...
module.exports = {
    ...
    // Get comments for a specific payment
    getPaymentComments: async (req, res) => {
        try {
            const { paymentId } = req.params;
            
            const query = `
                SELECT id, paymentId, comment, createdAt
                FROM paymentcomments
                WHERE paymentId = ?
                ORDER BY createdAt DESC
            `;
            
            const comments = await dbQuery(query, [paymentId]);
            
            res.json({
                success: true,
                data: comments,
                message: 'Payment comments retrieved successfully'
            });
        } catch (error) {
            console.error('Error fetching payment comments:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to fetch payment comments'
            });
        }
    },
    
    // Add a comment to a payment
    addPaymentComment: async (req, res) => {
        try {
            const { paymentId } = req.params;
            ...
            const paymentCheck = await dbQuery('SELECT id FROM payments WHERE id = ?', [paymentId]);
            ...
            const query = `
                INSERT INTO paymentcomments (paymentId, comment, createdAt)
                VALUES (?, ?, NOW())
            `;
            
            const result = await dbQuery(query, [paymentId, comment.trim()]);
            
            // Fetch the newly created comment
            const newComment = await dbQuery(
                'SELECT id, paymentId, comment, createdAt FROM paymentcomments WHERE id = ?',
                [result.insertId]
            );
            ...
            console.error('Error adding payment comment:', error);
            res.status(500).json({
                success: false,
                message: 'Failed to add payment comment'
            });
        }
    }
};
